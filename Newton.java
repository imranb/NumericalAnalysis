...
public class Newton{
	...
	public static double[][] functionDerivative(double x1,double x2,double x3)
	{
		double[][] res = new double[3][3];
		res[0][0] = 3;
	...
	public static double[][] newtonEquations(int N, double[][] x, double TOL)
	{
		...
		while(k<N)
		{
			double[][] inversedJ = MatrixInverseofRank3(functionDerivative(x[0][0],x[1][0],x[2][0]));
	...
	public static void main(String[] args)
	{
		double[][] A = {{0.1,0,0},	
						    {0.1,0,0},
					       {-0.1,0,0}};
		double[][] x = newtonEquations(6,A,0.000000000000000001);
		System.out.println("Zgjidhjet"+"\n------------------------------------------------");
		System.out.println("x1: "+x[0][0] + "\nx2: " +x[1][0] + "\nx3: " + x[2][0]);
		System.out.println("\nTestimi i saktësisë " + "\n------------------------------------------------");
		System.out.println("Ekuacioni i parë: "+(3.0*x[0][0] - Math.cos(x[1][0]*x[2][0]) - 1.0/2)+" = 0"); 
		System.out.println("Ekuacioni i dytë: "+(x[0][0]*x[0][0] - 81.0*Math.pow(x[1][0]+0.1,2) + Math.sin(x[2][0]) + 1.06)+" = 0"); 
		System.out.println("Ekuacioni i tretë: "+(Math.exp(-x[0][0]*x[1][0]) + 20.0*x[2][0] + (10*Math.PI - 3)/3.0)+" = 0"); 
	}
